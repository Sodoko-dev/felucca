...
The same `feluccad` and `felucca-agent` binaries run identically in the Lima dev
...
|---|---|---|---|
| `infra-saas-lab` | `192.168.104.3` | control plane | `feluccad-lab.json` below |
| `kata-lab-0` | `192.168.104.1` | worker 0 | `felucca-agent-lab.json` below |
...
start** on an empty token, a known placeholder (`felucca-lab-token`, anything
containing `REPLACE_WITH`), or a short one — under 32 characters for `feluccad`,
under 16 for `felucca-agent`. An unauthenticated agent API is root-level command
...
take it as `$1`/`TOKEN`/`FELUCCA_TOKEN` or fall back to that file
(`FELUCCA_TOKEN_FILE` overrides the path), and fail loudly when none is set.

Two things follow for lab work:

- **`/metrics` needs the token now.** `curl -s localhost:8080/metrics` returns
  `401`; add `-H "Authorization: Bearer $(cat ~/.config/felucca/lab-token)"`.
...
For a throwaway single-VM lab with nothing else on the network, `feluccad
--insecure-no-auth` skips the token entirely. It is loopback-and-lab only, it
must be named explicitly, feluccad logs a WARN every start, and there is no
`felucca-agent` equivalent — so an agent still needs a real token.

## feluccad — Lima lab (`/etc/felucca/feluccad.json` inside `infra-saas-lab`)

```json
{
  "bind": "0.0.0.0:8080",
  "state_path": "/var/lib/felucca/state.json",
  "ui_dir": "/usr/share/felucca/ui",
  "token": "<contents of ~/.config/felucca/lab-token>"
}
```
...
`bind` is `0.0.0.0` here and **that is deliberate for the lab only.** feluccad
...
no systemd units, so it has no `felucca-firewall` rules either: the L2-isolated
Lima network is the whole boundary. Do not expose a lab VM.

## felucca-agent — Lima lab (`/etc/felucca/felucca-agent.json` inside `kata-lab-0`)
...
run the systemd units, so it does not get the `felucca-firewall` rules that block
...
| `token` | 64-hex-char secret from `~/.config/felucca/lab-token` | 64-hex-char secret |
| TLS | none (plain HTTP, L2-isolated) | Caddy reverse proxy terminates TLS |
| `pool_size` | `0` (single-VM lab) | `2`–`5` per node |
| `data_dir` | `/srv/felucca` | `/srv/felucca` (same) |
| host firewall | not installed (no systemd in the lab) | `felucca-firewall.service`, required by `felucca-agent.service` |
