...
# cannot make hearthd buffer an arbitrary amount of the caller's choosing.
#
# The padding sits in a field hearthd ignores: minus the padding this is a
# VALID create that would return 201. That is what makes the case a real
# differential — it fails against a hearthd with no cap (201 + a live sandbox)
...
# ── Block 5: hearthd came through all of it ─────────────────────────────────
hd GET /healthz "" none
assert_status 200 "hearthd still healthy after the input-bound cases"
