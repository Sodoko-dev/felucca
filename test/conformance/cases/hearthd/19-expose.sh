...
# contract covers hearthd's wire surface; gateway behavior is pinned by its
...
if [ -n "$CF_EXP_T_KEY" ] && [ -n "$CF_EXP_ID" ]; then
  req_as "$CF_EXP_T_KEY" "$HEARTH_API" GET /api/v1/routes
  ...
  req_as "$CF_EXP_T_KEY" "$HEARTH_API" POST "/api/v1/sandboxes/$CF_EXP_ID/expose" '{"name":"steal","port":80}'
  ...
  req_as "$CF_EXP_T_KEY" "$HEARTH_API" POST /api/v1/routes/ensure "{\"sandbox_id\":\"$CF_EXP_ID\",\"port\":8069}"
  if [ "$R_STATUS" != "200" ]; then
