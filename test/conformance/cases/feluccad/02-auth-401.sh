...
# empty token authorizes nobody and feluccad refuses to start with one, so the
# only tokenless target is an explicit `feluccad --insecure-no-auth` lab, which
# the suite reaches through FELUCCA_INSECURE_NO_AUTH=1.
#
...
if [ "${FELUCCA_INSECURE_NO_AUTH:-0}" = "1" ]; then
  skip "FELUCCA_INSECURE_NO_AUTH=1; target is running --insecure-no-auth"
else
