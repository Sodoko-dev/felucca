...
#
# The scoped template is built by CAPTURE, not image_file: feluccad's images_dir
...
if [ -n "$CF_TV_SB" ]; then
  ...
  req_as "$CF_TV_A_KEY" "$FELUCCA_API" GET /api/v1/templates
  ...
  req_as "$CF_TV_B_KEY" "$FELUCCA_API" GET /api/v1/templates
  ...
  # (no existence leak).
  req_as "$CF_TV_B_KEY" "$FELUCCA_API" POST /api/v1/sandboxes "{\"name\":\"cf-tv-no-$CF_TV_RUN\",\"template\":\"$CF_TV_TPL\"}"
