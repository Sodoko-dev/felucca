...
# in body). Requires FELUCCA_TOKEN to be the admin key. Uses req_as for all
...
assert_jq '.api_key | startswith("felucca_sk_")' "cf-t1 api_key prefix"
...
assert_jq '.api_key | startswith("felucca_sk_")' "cf-t2 api_key prefix"
...
if [ -n "$CF_T1_ID" ]; then
  ...
  assert_jq '.api_key | startswith("felucca_sk_")' "rotated key has correct prefix"
...
# reconstruct it: `prefix` is the secret's first 15 chars ("felucca_sk_" + 4),
...
if [ -n "$CF_T1_ID" ]; then
  ...
  assert_jq '.keys | (map(select((.id | type) == "string" and (.prefix | length) == 15)) | length) == length' \
    "every listed key carries an id and a 15-char prefix"
  ...
  # The prefix really is the first 15 chars of the issued secret — that is what
  # makes it usable for matching a leak.
  cf_t1_pfx="${CF_T1_KEY:0:15}"
...
if [ -n "$CF_T1_KEY" ] && [ -n "$CF_T1_ID" ]; then
  ...
  req_as "$CF_T1_KEY" "$FELUCCA_API" GET "/api/v1/tenants/$CF_T1_ID/keys"
...
if [ -n "$CF_T1_KEY_EXP" ]; then
  sleep 2
  req_as "$CF_T1_KEY_EXP" "$FELUCCA_API" GET /api/v1/sandboxes
...
if [ -n "$CF_T1_KEY" ]; then
  req_as "$CF_T1_KEY" "$FELUCCA_API" GET /api/v1/tenants
  if [ "$R_STATUS" != "200" ]; then
...
if [ -n "$CF_T1_KEY2" ]; then
  req_as "$CF_T1_KEY2" "$FELUCCA_API" POST /api/v1/sandboxes \
...
CF_T1_SB_ID=""
if [ -n "$CF_T1_KEY" ]; then
  req_as "$CF_T1_KEY" "$FELUCCA_API" POST /api/v1/sandboxes \
...
if [ -n "$CF_T1_KEY" ] && [ -n "$CF_T1_SB_ID" ]; then
  req_as "$CF_T1_KEY" "$FELUCCA_API" GET /api/v1/sandboxes
...
CF_T2_SB_ID=""
if [ -n "$CF_T2_KEY" ]; then
  req_as "$CF_T2_KEY" "$FELUCCA_API" POST /api/v1/sandboxes \
...
if [ -n "$CF_T2_KEY" ] && [ -n "$CF_T1_SB_ID" ]; then
  req_as "$CF_T2_KEY" "$FELUCCA_API" GET "/api/v1/sandboxes/$CF_T1_SB_ID"
...
if [ -n "$CF_T2_KEY" ] && [ -n "$CF_T1_SB_ID" ]; then
  req_as "$CF_T2_KEY" "$FELUCCA_API" GET /api/v1/sandboxes
...
if [ -n "$CF_TQUOTA_KEY" ]; then
  req_as "$CF_TQUOTA_KEY" "$FELUCCA_API" POST /api/v1/sandboxes \
