...
# A path-traversal id is just another unmatched id: feluccad's route matcher
# refuses an id containing a separator, so the decoded "../../etc/felucca"
...
hd GET /api/v1/sandboxes/%2e%2e%2f%2e%2e%2fetc%2ffelucca
assert_status 404 "GET traversal-shaped sandbox id"
assert_body_exact '{"error":"not found"}' "traversal id body exact"
hd POST /api/v1/sandboxes/%2e%2e%2f%2e%2e%2fetc%2ffelucca/exec '{"cmd":["true"]}'
