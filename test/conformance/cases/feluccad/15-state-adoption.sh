...
# Local-only: a candidate feluccad binary must adopt the live state.json —
# same node ids and sandbox ids (agents never re-register, so losing node ids
# strands the fleet). Runs only when CANDIDATE_FELUCCAD points at a binary and
# the suite is running on the feluccad host itself.
if [ -z "${CANDIDATE_FELUCCAD:-}" ]; then
  skip "CANDIDATE_FELUCCAD not set (local-only case)"
elif [ ! -x "${CANDIDATE_FELUCCAD}" ]; then
  bad "CANDIDATE_FELUCCAD '$CANDIDATE_FELUCCAD' is not executable"
else
  cf_state_src="${ADOPT_STATE_PATH:-/var/lib/felucca/state.json}"
  cf_tmp=$(mktemp -d)
  ...
  # up owned by us because the candidate feluccad below runs as this user and
  ...
  if ! sudo cat "$cf_state_src" > "$cf_tmp/state.json" 2>/dev/null; then
    ...
    "$CANDIDATE_FELUCCAD" --bind 127.0.0.1:8099 --state "$cf_tmp/state.json" \
      --ui-dir /nonexistent --token "${FELUCCA_TOKEN:-}" \
    ...
    if [ -z "$cf_up" ]; then
      bad "candidate feluccad did not come up on :8099 ($(tail -1 "$cf_tmp/candidate.log" 2>/dev/null))"
    else
      cf_auth=()
      [ -n "${FELUCCA_TOKEN:-}" ] && cf_auth=(-H "Authorization: Bearer $FELUCCA_TOKEN")
      cf_live_nodes=$(curl -s -m5 "${cf_auth[@]}" "$FELUCCA_API/api/v1/nodes" | jq -S '[.nodes[].id] | sort')
      cf_cand_nodes=$(curl -s -m5 "${cf_auth[@]}" http://127.0.0.1:8099/api/v1/nodes | jq -S '[.nodes[].id] | sort')
      cf_live_sbs=$(curl -s -m5 "${cf_auth[@]}" "$FELUCCA_API/api/v1/sandboxes" | jq -S '[.sandboxes[].id] | sort')
