...
# GET /tenants/{id}/usage. The feluccad sweep runs every 15s, so each policy
...
if [ -n "$CF_LC_T_KEY" ]; then
  req_as "$CF_LC_T_KEY" "$FELUCCA_API" POST /api/v1/sandboxes "{\"name\":\"cf-lc-sb-$CF_LC_RUN\"}"
...
cf_lc_wait_state() { # cf_lc_wait_state <key> <id> <state> <tries-of-3s>
  local i=0
  while [ "$i" -lt "$4" ]; do
    req_as "$1" "$FELUCCA_API" GET "/api/v1/sandboxes/$2"
...
if [ -n "$CF_LC_SB" ]; then
  ...
  req_as "$CF_LC_T_KEY" "$FELUCCA_API" POST "/api/v1/sandboxes/$CF_LC_SB/wake"
  ...
  sleep 4
  req_as "$CF_LC_T_KEY" "$FELUCCA_API" POST "/api/v1/sandboxes/$CF_LC_SB/exec" '{"cmd":["true"],"timeout_ms":5000}'
  sleep 4
  req_as "$CF_LC_T_KEY" "$FELUCCA_API" GET "/api/v1/sandboxes/$CF_LC_SB"
...
CF_LC_SB2=""
if [ -n "$CF_LC_T_KEY" ]; then
  req_as "$CF_LC_T_KEY" "$FELUCCA_API" POST /api/v1/sandboxes "{\"name\":\"cf-lc-never-$CF_LC_RUN\",\"idle_sleep_s\":-1}"
  if [ "$R_STATUS" = "201" ]; then
...
if [ -n "$CF_LC_SB2" ]; then
  ...
  sleep 30
  req_as "$CF_LC_T_KEY" "$FELUCCA_API" GET "/api/v1/sandboxes/$CF_LC_SB2"
  if [ "$(printf '%s' "$R_BODY" | jq -r .state)" = "running" ]; then
...
CF_LC_SB3=""
if [ -n "$CF_LC_T_KEY" ]; then
  req_as "$CF_LC_T_KEY" "$FELUCCA_API" POST /api/v1/sandboxes "{\"name\":\"cf-lc-ttl-$CF_LC_RUN\",\"idle_sleep_s\":5,\"asleep_delete_s\":30}"
  if [ "$R_STATUS" = "201" ]; then
...
if [ -n "$CF_LC_SB3" ]; then
  ...
  for _ in $(seq 1 25); do
    req_as "$CF_LC_T_KEY" "$FELUCCA_API" GET "/api/v1/sandboxes/$CF_LC_SB3"
...
if [ -n "$CF_LC_T_ID" ]; then
  ...
  req_as "$CF_LC_T_KEY" "$FELUCCA_API" GET "/api/v1/tenants/$CF_LC_T_ID/usage"
  if [ "$R_STATUS" = "200" ]; then
    ok "tenant key reads its own usage"
  else
    bad "tenant self usage: expected 200, got $R_STATUS"
  fi
  req_as "$CF_LC_T_KEY" "$FELUCCA_API" GET "/api/v1/tenants/tn-other/usage"
