...
# the authenticated admin metrics surface, and X-Felucca-Request-Id minting.
...
for cf_ob_h in felucca_wake_duration_ms felucca_exec_duration_ms felucca_create_duration_ms; do
...
if printf '%s' "$R_BODY" | grep -q "felucca_tenant_"; then
  bad "tenant-labeled series leaked onto the fleet /metrics"
else
  ok "no felucca_tenant_* on the fleet /metrics (leak guard)"
fi
# Legacy wake counters stay (pre-P6 dashboards).
if printf '%s' "$R_BODY" | grep -q "^felucca_wake_ms_sum"; then
  ok "legacy felucca_wake_ms_sum still emitted"
else
  bad "legacy felucca_wake_ms_sum dropped"
fi

# ── Block 2: admin metrics surface auth ladder ──────────────────────────────
if [ "${FELUCCA_INSECURE_NO_AUTH:-0}" = "1" ]; then
  skip "FELUCCA_INSECURE_NO_AUTH=1; admin-surface auth ladder needs a credential"
else
...
if printf '%s' "$R_BODY" | grep -q "felucca_tenant_sandboxes"; then
  ok "admin surface serves felucca_tenant_sandboxes"
else
  bad "admin surface missing felucca_tenant_sandboxes: $(printf '%s' "$R_BODY" | head -c 200)"
fi
...
if [ -n "$CF_OB_KEY" ]; then
  req_as "$CF_OB_KEY" "$FELUCCA_API" GET /api/v1/metrics/tenants
  ...
  req_as "$CF_OB_KEY" "$FELUCCA_API" GET /metrics
  if [ "$R_STATUS" = "404" ]; then
...
cf_ob_h1=$(curl -s -m 10 -D - -o /dev/null -H "Authorization: Bearer $FELUCCA_TOKEN" \
  "$FELUCCA_API/api/v1/sandboxes" | grep -i '^x-felucca-request-id:' | tr -d '\r' | awk '{print $2}')
cf_ob_h2=$(curl -s -m 10 -D - -o /dev/null -H "Authorization: Bearer $FELUCCA_TOKEN" \
  "$FELUCCA_API/api/v1/sandboxes" | grep -i '^x-felucca-request-id:' | tr -d '\r' | awk '{print $2}')
case "$cf_ob_h1" in
  req-*) ok "X-Felucca-Request-Id minted with req- prefix ($cf_ob_h1)" ;;
  *)     bad "X-Felucca-Request-Id missing or malformed: '$cf_ob_h1'" ;;
esac
