...
# distribution path — the worker pulls the captured image from feluccad and
...
if [ -n "$CF_TPL_SB" ]; then
  ...
  # The capture streams the full 2 GiB rootfs agent→feluccad and hashes it —
...
if [ -n "$CF_TPL_SB" ]; then
  ...
  # The worker may need to pull the 2 GiB image from feluccad first (prefetch
  # is async, and a cold-cache create 502s by design when the pull outlives
  # feluccad's 30s agent call — ADR-0008). Retry until the cache is warm.
...
if [ -n "$CF_TPL_T_KEY" ]; then
  req_as "$CF_TPL_T_KEY" "$FELUCCA_API" GET /api/v1/templates
  ...
  req_as "$CF_TPL_T_KEY" "$FELUCCA_API" POST /api/v1/templates '{"name":"cf-t-no","image_file":"ubuntu-base"}'
  ...
  req_as "$CF_TPL_T_KEY" "$FELUCCA_API" GET "/api/v1/images/ubuntu-base"
  if [ "$R_STATUS" = "404" ]; then
  ...
  CF_TPL_Q1=""
  req_as "$CF_TPL_T_KEY" "$FELUCCA_API" POST /api/v1/sandboxes "{\"name\":\"cf-tpl-q1-$CF_RUN\",\"disk_gb\":4}"
  ...
  req_as "$CF_TPL_T_KEY" "$FELUCCA_API" POST /api/v1/sandboxes "{\"name\":\"cf-tpl-q2-$CF_RUN\"}"
  ...
  if [ -n "$CF_TPL_Q1" ]; then
    req_as "$CF_TPL_T_KEY" "$FELUCCA_API" DELETE "/api/v1/sandboxes/$CF_TPL_Q1"
