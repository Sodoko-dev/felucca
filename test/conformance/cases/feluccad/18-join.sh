...
#
# This case NEVER completes a successful join on purpose: the lab feluccad runs
...
CF_UNKNOWN_JT="felucca_jt_0123456789abcdef0123456789abcdef0123456789abcdef"
...
assert_jq '.token | startswith("felucca_jt_")' "minted token prefix"
...
if [ -n "$CF_JOIN_T_KEY" ]; then
  req_as "$CF_JOIN_T_KEY" "$FELUCCA_API" POST /api/v1/join-tokens '{"node_hint":"nope"}'
...
req_as "felucca_sk_not_a_join_token" "$FELUCCA_API" POST /api/v1/nodes/join \
  "{\"pubkey\":\"$CF_DEAD_PUBKEY\",\"hostname\":\"cf-join\"}"
assert_status 401 "POST /api/v1/nodes/join with felucca_sk_-shaped bearer -> 401"
assert_body_exact '{"error":"unauthorized"}' "wrong-credential-kind 401 body exact"

# Well-formed but unknown felucca_jt_ token + VALID-shape body: must reach the
# token check and 401 there. Nothing is consumed and no peer is created.
req_as "$CF_UNKNOWN_JT" "$FELUCCA_API" POST /api/v1/nodes/join \
  "{\"pubkey\":\"$CF_DEAD_PUBKEY\",\"hostname\":\"cf-join\"}"
assert_status 401 "POST /api/v1/nodes/join with unknown felucca_jt_ token -> 401"
...
if [ -n "$CF_JT_TOKEN" ]; then
  req_as "$CF_JT_TOKEN" "$FELUCCA_API" POST /api/v1/nodes/join \
