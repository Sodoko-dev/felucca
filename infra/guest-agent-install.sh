#!/usr/bin/env bash
# Install the felucca-guest agent into the base rootfs image on a worker.
#
...
# then writes the marker ${DATA_DIR}/images/.felucca-guest-v1 NEXT TO the image
# (outside it) — the node agent keys off that marker.
#
# Usage:  guest-agent-install.sh [path-to-felucca-guest]   (default /tmp/felucca-guest)
#
...
BIN_SRC="${1:-/tmp/felucca-guest}"
DATA_DIR="${DATA_DIR:-/srv/ignis}"
IMAGE="${DATA_DIR}/images/ubuntu-base.ext4"
MARKER="${DATA_DIR}/images/.felucca-guest-v1"

UNIT_PATH="/etc/systemd/system/felucca-guest.service"
WANTS_LINK="/etc/systemd/system/multi-user.target.wants/felucca-guest.service"
...
MNT="$(mktemp -d /tmp/felucca-guest-mnt.XXXXXX)"
...
echo "==> Installing binary -> /usr/local/bin/felucca-guest"
install -D -m 0755 "${BIN_SRC}" "${MNT}/usr/local/bin/felucca-guest"

echo "==> Writing systemd unit -> ${UNIT_PATH}"
install -d -m 0755 "${MNT}/etc/systemd/system"
cat >"${MNT}${UNIT_PATH}" <<'UNIT'
[Unit]
Description=Felucca guest agent (vsock exec + re-IP)

[Service]
ExecStart=/usr/local/bin/felucca-guest
Restart=always
RestartSec=2
...
echo "==> Done: felucca-guest installed into base image; marker written."
