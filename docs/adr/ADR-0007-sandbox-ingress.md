# ADR-0007 — Sandbox ingress: named exposes, worker DNAT, hearth-gw
...
2. **An expose is a route row plus one worker DNAT rule.** hearthd stores
   ...
   chain (nat hook prerouting, priority -100) of the existing `ip hearth`
...
3. **The gateway is a separate small binary (`hearth-gw`).** Wildcard DNS
   `*.<domain>` points at it; it maps Host labels to `node_host:node_port`
   using hearthd's admin-only `GET /api/v1/routes` (cached, refreshed every
   5s; a hearthd outage keeps the last table — routes must not die with the
   control plane). It dials workers exactly like hearthd does (direct or
...
4. **Dynamic ports are lazy exposes, off by default.** `8069--<id>` hosts
   ...
   an unauthenticated request-amplification loop against hearthd).
...
6. **Per-tenant ingress policy lives at the ingress.** `hearth-gw` takes a
   `--tenant-limits` JSON map (enabled toggle + token-bucket rps) — not
   hearthd store state. Rationale: limits are an edge concern, vary per
   gateway deployment, and need no API round-trip on the hot path.
7. **Ingress mutations are serialized in hearthd** (`exposeMu`, outer to the
...
- **Routing in hearthd itself.** Couples the data plane to the control
  plane's availability and load profile; a gateway is deployable N× behind
  DNS/LB and beside the traffic.
- **Per-request route lookups against hearthd.** An API round-trip per HTTP
...
- The gateway holds the hearthd admin token (the route table is admin-only)
  — its env file is root:hearth 0640 like hearthd's own config.

## Deferred (recorded during the P3 review loop)

- **hearthd↔agent expose reconciliation**: a best-effort sandbox delete that
