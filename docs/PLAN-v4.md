...
> NAT'd home-lab workers. Felucca's measured position (BENCHMARKS.md, nested-virt
...
First consumer: **Sodoko** (`../odoo-engineer-kosuke`) — Lovable-for-Odoo. Each sandbox runs a full self-hosted Odoo system (odoo v17/18/19-CE + postgres + agent-server, extendable with Traccar/Chatwoot) as a docker-compose stack *inside* the microVM, with AI agents exec'ing in-guest and end users reaching the Odoo web UI via per-sandbox public URLs (today: traefik `dynamic-sandboxes.yml`; sandboxes prototyped on Sprites — Felucca replaces that). Direction may broaden to more products/users, so everything is built tenant-generic.
...
- Auth: one shared bearer token on both sides (`check_auth` in `rust/agent/src/server.rs`; equivalent in Go). feluccad→agent uses the same token over plain HTTP (`go/internal/agentclient`).
- `rust/agent/src/net.rs` — `ip felucca` nft table with a **postrouting masquerade chain only**; no forward filtering → guests on one bridge can reach each other freely today. Shell-out style (`ip`/`nft`/`sysctl`) is the established pattern.
- `rust/agent/src/config.rs` + `go/internal/config` — flags > env (`FELUCCA_*`) > JSON > defaults; new keys slot in cleanly.
...
- Build constraint: `CGO_ENABLED=0` static feluccad (DEPLOYMENT.md) — SQLite must be pure-Go (`modernc.org/sqlite`), not mattn/cgo.
...
conformance **166/0** (new `feluccad/17-tenancy`, +34 checks), verify-v2 **21/0**,
...
2. **Tenant + APIKey entities**: `tenants` (id, name, quotas), `api_keys` (hash — store SHA-256 only, prefix `felucca_sk_`, tenant_id, created/revoked). Admin bootstrap key from config. CRUD endpoints under `/api/v1/tenants` (admin-key only).
...
4. **Quotas**: per-tenant max sandboxes / vcpus / mem_mib / disk_gb, checked at create/fork; 429 + `felucca_quota_rejections_total` metric.
5. **Usage metering (events now, billing later)**: `usage_events` table — one row per sandbox lifecycle transition (tenant_id, sandbox_id, event, shape vcpus/mem/disk, timestamp), written where feluccad already applies state changes. Append-only; aggregation comes in P5. Records exist from day one so any future billing can be computed retroactively.

Conformance: new `feluccad/17-tenancy` cases (key scoping, cross-tenant 404, quota 429). The existing single-token mode stays as "admin key" so all current cases keep passing.
...
1. Add a `forward` chain (hook forward, policy drop for bridge-to-bridge) in the existing `ip felucca` table: allow established/related, allow guest→egress (non-CIDR destinations), **drop guest→guest by default**.
2. Per-tenant nft **named sets** (`felucca_t_<tenant>`): member IPs added/removed on VM create/delete/IP-change (fork re-IP included); one rule allows intra-set traffic. Rebuilt idempotently on agent start from adopted metas (reuse the reconcile pass).
...
> a public host for feluccad (small cloud VM with a public IP), at least one
...
1. **Overlay**: feluccad gains a `wg` config block (its overlay IP, listen port, key path). New `POST /api/v1/nodes/join` exchanging a one-time **join token** (admin-issued, `felucca join-token create`) for: overlay address assignment, feluccad's pubkey/endpoint, peer registration. Agent gains `--join <url> --token <t>`: generates keys, calls join, writes/starts a `wg` interface (shell-out to `wg`/`ip`, consistent with net.rs), then registers as today over the overlay IP. NAT'd workers: persistent-keepalive; only feluccad needs a public endpoint.
2. **Public API TLS**: Go `autocert` (Let's Encrypt) on feluccad's public listener — keeps the single-binary story; `--tls-domain` config. UI served over the same listener.
...
4. **Mixed-fleet acceptance**: one Hetzner bare-metal worker + one nested-virt cloud VM + one home-lab/Lima worker joined to a cloud feluccad — `verify-v2` must pass against that fleet unmodified. This test *is* the positioning claim.
...
3. **Gateway component**: a small Go reverse proxy (`cmd/felucca-gw`, reusing config/auth packages) deployable anywhere (typically beside feluccad): wildcard DNS `*.sb.<domain>` → gateway; ACME DNS-01 wildcard cert; routes from feluccad's API (cached, event-refreshed). Runs over the WG overlay to reach workers — so a sandbox on a NAT'd home-lab box is still publicly reachable. **WebSocket upgrade passthrough is in the acceptance tests from day one** (Odoo longpolling/chat, Chatwoot live updates; multi-worker Odoo's 8072 is just a second expose). Sleeping sandbox → 503 wake-on-request page (later: auto-wake).
...
`feluccad/20-templates`, +39 checks incl. a capture → cross-node pull →
...
(new `feluccad/21-stream-exec`, `22-lifecycle`, `23-template-visibility`),
verify-v2 **21/0**, cargo **41/0 guest + 121/0 agent**, go suite green, static
feluccad+felucca-gw link. ADR-0009 + API-V2 §3f written. Also closed the
...
→ SSE at feluccad (one wire shape, translated per hop), buffered exec
...
stdin/PTY, `felucca` CLI binary, authenticated metrics, per-sandbox policy
PATCH, SDK npm publish.

1. **Streaming exec**: SSE on feluccad (`?stream=1`) — agent streams stdout/stderr chunks over the existing vsock protocol (protocol gains a `stream` op; felucca-guest writes incremental frames). Long-running agent commands (module installs, builds) need this; buffered exec stays for compat.
2. **Idle/TTL policies**: per-tenant defaults + per-sandbox overrides — auto-sleep after N min idle (no exec/ingress traffic), optional auto-delete after M days asleep. Reconcile loop in feluccad drives it (it owns last-activity timestamps from exec/ingress).
3. **TypeScript SDK** (`sdk/ts/`): thin typed client over REST — create/exec(stream)/sleep/wake/fork/expose; mirrors the conformance contract so Sodoko integrates in a day. Publish the conformance suite as `felucca verify <endpoint>` — the trust artifact for self-hosters.
4. **Usage aggregation** over P0's `usage_events`: `GET /api/v1/tenants/{id}/usage?from&to` → sandbox-hours, vCPU-hours, GB-hours, exec counts (computed from transition pairs); `felucca_tenant_*` gauges for dashboards. Billing integration stays out of scope — this endpoint is its future input.

## Phase 6 — Scale-out & observability — ✅ DONE 2026-08-09

Completed and committed. All five gates green: conformance **342/0** (new
`feluccad/24-observability`), verify-v2 **21/0**, cargo **121/0 agent +
...
- Structured logging (Go `slog`, Rust `tracing`) replacing eprintln/stderr; request IDs across feluccad→agent.
...
- HA path when needed: `Store` already abstracts — litestream replication first, Postgres swap second, N×feluccad behind LB (ADR-0002's sanctioned path).
...
1. **Unit & build gate** — all inside the toolchain VM, never the host:
   ...
   static `CGO_ENABLED=0` cross-build of feluccad still links (the single-binary
...
2. **Deploy gate** — roll the phase's binaries to the lab via the runbook
   (agents first, then feluccad): healthz on all three, 2 nodes ready, state
...
- Every phase adds conformance cases; the suite remains the contract (`test/conformance/`), runnable against any deployment (`felucca verify`).
