// Structural test for feluccad's listeners. main() binds real ports and blocks,
...
func TestEveryListenerIsBounded(t *testing.T) {
	...
	if servers < 3 {
		t.Fatalf("found only %d http.Server literals; feluccad has three listeners, so the audit is not seeing them all", servers)
	}
}

func TestLegacyDBPath(t *testing.T) {
	dir := t.TempDir()
	oldDir := filepath.Join(dir, "hearth")
	if err := os.MkdirAll(oldDir, 0o755); err != nil {
		t.Fatal(err)
	}
	old := filepath.Join(oldDir, "hearth.db")
	if err := os.WriteFile(old, []byte("x"), 0o600); err != nil {
		t.Fatal(err)
	}
	if got := legacyDBPath(filepath.Join(dir, "felucca", "felucca.db")); got != old {
		t.Errorf("legacyDBPath: got %q, want %q", got, old)
	}
	if got := legacyDBPath(filepath.Join(dir, "felucca", "other.db")); got != "" {
		t.Errorf("no legacy file: got %q, want empty", got)
	}
	if got := legacyDBPath(filepath.Join(dir, "state.db")); got != "" {
		t.Errorf("path without product name: got %q, want empty", got)
	}
}
