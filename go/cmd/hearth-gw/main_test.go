// Structural test for hearth-gw's listener, mirroring hearthd's
// TestEveryListenerIsBounded (cmd/hearthd/main_test.go). main() binds a real
...
// hardening did not reach, and it is the INTERNET-FACING one: hearth-gw is the
// tenant ingress. It set ReadHeaderTimeout and nothing else — Go's 1 MiB
// per-connection header default, no idle bound.
//
// The required set is NOT hearthd's, and the difference is the point. hearth-gw
...
func TestIngressListenerIsBounded(t *testing.T) {
	...
	if servers < 1 {
		t.Fatal("found no http.Server literal; hearth-gw serves ingress, so the audit is not seeing its listener")
	}
}
